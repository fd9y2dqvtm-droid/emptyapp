...
class Config:
    ...
    # 安装包存放的服务器的域名 --- 必填项
    DeployDomain = '192.168.11.232:8000'  # 使用您的本地IP和端口

    # 安装包存放的服务器地址，必须是 https --- 必填项
    # 注意：本地开发可以用http，但生产环境建议用https
    ...
    # 打包签名配置
    Alias = 'bieming'  # --- 必填项
    KeyPwd = 'Abcd@123456'  # --- 必填项
    KeystorePwd = 'Abcd@123456'  # --- 必填项
    # 替换 hapck/sign 目录下的证书文件，如替换的文件名一致，则不用修改
    SignDir = 'sign'
    Cert = os.path.join(SignDir, 'certificate.cer')
    Profile = os.path.join(SignDir, 'pfDebug.p7b')
    Keystore =  os.path.join(SignDir, 'miyao.p12')
    ...
    HistoryBtnUrl = "http://192.168.11.232:8000/history.html" # 历史版本页面url地址